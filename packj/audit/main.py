...
def msg_warn(x):
    msg_info(
        f'{Style.BRIGHT}{Fore.YELLOW} N/A{Style.RESET_ALL} [{Fore.MAGENTA}{x}{Style.RESET_ALL}]'
    )
...
def analyze_install_hooks(pm_name, pm_proxy, pkg_name, pkg_info, risks, report):
    ...
    try:
        ...
        if language == LanguageEnum.python:
            ...
            results = pm_proxy.get_install_hooks(pkg_name, pkg_info=pkg_info).keys()
            count = len(results)
            if count > 0:
                is_ok = False
                msg_alert(f'has {count} install hook(s): {",".join(results)}')
...
def audit(pm_args, pkg_name, ver_str, report_dir, extra_args, config):
    ...
    risks, report = analyze_install_hooks(
        pm_name, pm_proxy, pkg_name, pkg_info, risks, report
    )
