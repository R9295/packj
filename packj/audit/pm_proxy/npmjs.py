...
class NpmjsProxy(PackageManagerProxy):
    ...
    def get_release_history(self, pkg_name, pkg_info=None, max_num=-1):
        ...
        for ver_str, ver_info in pkg_info['versions'].items():
            ...
            try:
                ...
                if (
                    descr == 'security holding package'
                    and repo == 'npm/security-holder'
                    and version == '0.0.1-security'
                ):
                    yanked = True
            except:
                yanked = None
    ...
    def get_dependencies(self, pkg_name, ver_str=None, pkg_info=None, ver_info=None):
        try:
            ...
            logging.debug(
                "error parsing %s (%s) dependencies: %s" % (pkg_name, ver_str, str(e))
            )
            return None
    ...
    def __parse_dev_list(self, dev_list: str, dev_type: str, data=None):
        if not dev_list:
            ...
            isinstance(dev_list, list)
            and len(dev_list)
            and isinstance(dev_list[0], dict)
        ):
            pass
    ...
    def get_install_hooks(
        ...
        assert ver_info, "invalid metadata!"
        results = {}
        if ver_info.get('scripts'):
            scripts = ver_info.get('scripts')
            for hook in self._INSTALL_SCRIPTS:
                if scripts.get(hook):
                    results[hook] = scripts.get(hook)
        return results
