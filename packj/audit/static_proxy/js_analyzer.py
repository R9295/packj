...
class JavaScriptDeclRefVisitor(esprima.NodeVisitor):
    def __init__(self, infile, source, configpb=None, debug=False):
        ...
        self.declrefs = []
        self.all_declrefs = {"Calls": [], "Functions": [], "EmptyCatchClause": []}

    def visit_FunctionDeclaration(self, node):
        logging.debug(
            'visiting FunctionDeclaration node in file %s: %s' % (self.infile, node)
        )
        esprima.NodeVisitor.generic_visit(self, node)

    def visit_ArrowFunctionExpression(self, node):
        logging.debug(
            'visiting ArrowFunctionExpression node in file %s: %s' % (self.infile, node)
        )
        esprima.NodeVisitor.generic_visit(self, node)

    def visit_FunctionExpression(self, node):
        logging.debug(
            'visiting FunctionExpression node in file %s: %s' % (self.infile, node)
        )
    ...
    def visit_TryStatement(self, node):
        if (
            len(node.handler.body.body) == 0
        ):
            node_details = {
                "Name": "",
                "File": self.infile,
                "StartLine": node.block.loc.start.line,
                "EndLine": node.block.loc.end.line,
            }
    ...
    def visit_NewExpression(self, node):
        logging.debug(
            'visiting NewExpression node in file %s: %s' % (self.infile, node)
        )
        ...
        try:
            ...
            if node.callee.type == "Identifier":
                ...
                logging.warning(
                    "callee %s with node.callee.type %s is unhandled!",
                    name,
                    node.callee.type,
                )
            ...
            # log stuff
            logging.warning(
                "calling function %s with args %s at line %d"
            ...
            if (
                ...
                self.declrefs.append(
                    (base, name, tuple(args), source_text, source_range)
                )
        except:
            pass
    ...
    def visit_CallExpression(self, node):
        logging.debug(
            'visiting CallExpression node in file %s: %s' % (self.infile, node)
        )
        ...
        try:
            ...
            if node.callee.type == "Identifier":
                ...
                if node.callee.property.type == "Identifier":
                    ...
                    logging.warning(
                        "node.callee.property.type is %s!", node.callee.property.type
                    )
                if node.callee.object.type == "Identifier":
                    ...
                    logging.warning(
                        "node.callee.object.type is %s!", node.callee.object.type
                    )
            else:
                base = 'dummy'
                name = get_source_text(self.source, node.callee.loc)
...
class JsAnalyzer(StaticAnalyzer):
    ...
    def astgen(
        ...
        # get input files
        allfiles, infiles, bins, root = self._get_infiles(
            inpath=analyze_path, root=root, language=self.language
        )
        ...
        for infile in allfiles:
            ...
            try:
                ...
                logging.debug(
                    "Failed to parse FILE %s ast details: %s" % (infile, str(e))
                )
            ...
            try:
                ...
                logging.warning(
                    "Fatal error %s parsing file %s! Skipping this file!", e, infile
                )
                continue

            try:
                visitor = JavaScriptDeclRefVisitor(
                    infile=infile, source=all_source, configpb=configpb
                )
                visitor.visit(tree)
                logging.warning(
                    "collected functions: %s", Counter(visitor.get_declrefs()).items()
                )
                ...
                logging.debug(
                    "Error parsing AST for file %s in Python3: %s" % (infile, str(se))
                )
        ...
        # clean up residues
        self._cleanup_astgen(
            analyze_path=analyze_path, is_decompress_path=is_decompress_path
        )
